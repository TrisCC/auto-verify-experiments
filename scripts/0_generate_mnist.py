...
def load_data(data_dir: str = "./tmp", num_imgs: int = 25, random: bool = True) -> tuple:
    ...
    # sess1 = onnxrun.InferenceSession("./mnist_fc_data/onnx/mnist-net_256x2.onnx")
    # sess2 = onnxrun.InferenceSession("./mnist_fc_data/onnx/mnist-net_256x4.onnx")
    sess3 = onnxrun.InferenceSession("./mnist_fc_single_datasets/mnist_fc_single_train/onnx/mnist-net_256x6.onnx")
...
def create_instances_csv(num_props: int = 15, path: str = "mnistfc_instances.csv"):
    ...
    # nets = ["onnx/mnist-net_256x2.onnx",
    #         "onnx/mnist-net_256x4.onnx",
    #         "onnx/mnist-net_256x6.onnx"]
    
    nets = ["onnx/mnist-net_256x6.onnx"]

    props = [f"vnnlib/prop_{i}_0.03.vnnlib" for i in range(num_props)]
    props += [f"vnnlib/prop_{i}_0.05.vnnlib" for i in range(num_props)]

    with open(path, "w") as f:

        for net in nets:
            timeout = 120 if net == "onnx/mnist-net_256x2.onnx" else 300
            for prop in props:
...
if __name__ == '__main__':
    ...
    for eps in epsilons:
        for i in range(num_images):
            ...
            spec_path = f"./mnist_fc_single_datasets/mnist_fc_single_train/vnnlib/prop_{i}_{eps:.2f}.vnnlib"

            save_vnnlib(input_bounds, label, spec_path)

    create_instances_csv(num_props=num_images, path="./mnist_fc_single_datasets/mnist_fc_single_train/instances.csv")